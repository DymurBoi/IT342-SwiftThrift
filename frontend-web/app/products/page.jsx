...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    async function fetchWishlist() {
      try {
        const res = await axios.get(`http://localhost:8080/api/wishlist/byUser/${user.userId}`)
        if (res.data && res.data.length > 0) {
          setWishlistId(res.data[0].id || res.data[0].wishlistId)
          const itemsRes = await axios.get(`http://localhost:8080/api/wishlistItems/wishlist/${res.data[0].id || res.data[0].wishlistId}`)
          setWishlistItems(itemsRes.data.map(item => item.product.productId))
        } else {
          const createRes = await axios.post("http://localhost:8080/api/wishlist/create", {
  ...
  const handleWishlistToggle = async (productId) => {
    ...
    try {
      if (!isInWishlist) {
        await axios.post("http://localhost:8080/api/wishlistItems/create", {
        ...
        const res = await axios.get(`http://localhost:8080/api/wishlistItems/wishlist/${wishlistId}`, {
          withCredentials: true
        })
        const item = res.data.find(i => i.product.productId === productId)
        if (item) {
          await axios.delete(`http://localhost:8080/api/wishlistItems/${item.wishlistid || item.id}`, {
            withCredentials: true
          })
        }
      }
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          const allCartsRes = await axios.get("http://localhost:8080/api/cart/all")
          ...
          if (userExistingCart) {
            ...
            const createRes = await axios.post("http://localhost:8080/api/cart/create", {
              user: { userId: user.userId },
              totalPrice: 0
            })
      ...
      await axios.post("http://localhost:8080/api/cartItem/create", {
        price: product.price,
            ...
            return (
              <div key={product.productId} className="product-card">
                ...
                <div className="product-info">
                  ...
                  <p className="price">₱{product.price?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                  <button
