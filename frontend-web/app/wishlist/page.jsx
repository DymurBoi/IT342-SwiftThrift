...
export default function WishlistPage() {
  ...
  async function fetchWishlist(userId) {
    try {
      setLoading(true)
      const res = await axios.get(`http://localhost:8080/api/wishlist/byUser/${userId}`)
  ...
  const handleRemove = async (itemId) => {
    try {
      await axios.delete(`http://localhost:8080/api/wishlistItems/${itemId}`)
            ...
            wishlistItems.map((item) => (
              <div key={item.wishlistItemId || item.id} className="product-card">
                ...
                <div className="product-info">
                  ...
                  <p className="price">₱{item.product.price?.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
